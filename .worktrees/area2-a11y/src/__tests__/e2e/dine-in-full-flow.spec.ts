...
test.describe('Dine-in Full Flow', () => {
  ...
  test('login as Waiter and open shift', async ({ page }) => {
    ...
    // TODO: await page.getByRole('button', { name: 'Waiter' }).click()

    // Step 3: Enter staff name
    // TODO: await page.getByPlaceholder('ชื่อพนักงาน').fill('Test Staff')

    // Step 4: Enter branch name
    // TODO: await page.getByPlaceholder('สาขา').fill('A Ramen Ekkamai')

    // Step 5: Enter opening cash and submit shift open
    // TODO: await page.getByRole('button', { name: 'เริ่มกะ' }).click()

    // Step 6: Assert redirected to table-map
    // TODO: await expect(page).toHaveURL('/table-map')

    // Step 7: Assert header shows shift info
    // TODO: await expect(page.getByText('A Ramen Ekkamai')).toBeVisible()
  })
  ...
  test('open table T1 with guest count', async ({ page }) => {
    ...
    // TODO: await expect(page.getByRole('tab', { name: 'Dine-in' })).toHaveAttribute('data-state', 'active')

    // Step 3: Click an Open table tile (e.g. T1)
    // TODO: await page.getByTestId('table-tile-T1').click()
    // TODO: // Alternative: await page.getByText('T1').first().click()
    ...
    // TODO: // Alternative: await page.getByRole('spinbutton').fill('2')
    ...
    // TODO: await expect(page.getByTestId('table-tile-T1')).toHaveAttribute('data-status', 'Occupied')
    // TODO: // Alternative: await expect(page.getByTestId('table-tile-T1')).toContainText('Occupied')
  })
  ...
  test('add order items with ramen modifiers', async ({ page }) => {
    ...
    // TODO: await page.goto('/order/T1')

    // Step 2: Assert header shows table label + guest count
    // TODO: await expect(page.getByText('T1')).toBeVisible()
    // TODO: await expect(page.getByText('2 คน')).toBeVisible()
    ...
    // TODO: await page.getByTestId('menu-item-ramen-001').click()
    // TODO: // Alternative: await page.getByText('Tonkotsu Ramen').click()
    ...
    // TODO: await page.getByRole('button', { name: 'Normal' }).click()

    // Step 8: Select spice level (e.g. 2 flames)
    // TODO: await page.getByTestId('spice-level-2').click()
  ...
  test('send order to kitchen', async ({ page }) => {
    ...
    // TODO: await expect(page.getByTestId('ticket-panel')).toBeVisible()
    // TODO: await expect(page.getByTestId('ticket-panel').getByText('Tonkotsu Ramen')).toBeVisible()
    ...
    // TODO: await expect(page.getByText('ส่งออเดอร์แล้ว')).toBeVisible()

    // Step 6: Assert ticket items show "sent" status
    // TODO: await expect(page.getByTestId('ticket-item-sent')).toBeVisible()
  })
  ...
  test('KDS bump ticket through stages', async ({ page }) => {
    ...
    // TODO: await expect(page.getByTestId('kds-ticket-T1')).toBeVisible()
    // TODO: await expect(page.getByTestId('kds-ticket-T1')).toContainText('New')

    // Step 4: Click BUMP to advance to InProgress
    // TODO: await page.getByTestId('kds-ticket-T1').getByRole('button', { name: 'BUMP' }).click()

    // Step 5: Assert ticket moves to InProgress column
    // TODO: await expect(page.getByTestId('kds-ticket-T1')).toContainText('InProgress')

    // Step 6: Check all items as cooked (required before final BUMP)
    // TODO: await page.getByTestId('kds-ticket-T1').getByRole('checkbox').first().check()

    // Step 7: BUMP again to advance to Ready
    // TODO: await page.getByTestId('kds-ticket-T1').getByRole('button', { name: 'BUMP' }).click()

    // Step 8: Assert ticket moves to Ready column
    // TODO: await expect(page.getByTestId('kds-ticket-T1')).toContainText('Ready')

    // Step 9: Navigate back to table-map and assert T1 shows Ready badge
    // TODO: await page.goto('/table-map')
    // TODO: await expect(page.getByTestId('table-tile-T1')).toContainText('Ready')
  })
  ...
  test('confirm payment and close table', async ({ page }) => {
    ...
    // TODO: await page.goto('/payment/T1')

    // Step 2: Assert bill is visible with correct items
    // TODO: await expect(page.getByTestId('bill-line-item')).toBeVisible()

    // Step 3: Assert totals section shows subtotal + VAT
    // TODO: await expect(page.getByText('ยอดรวม')).toBeVisible()
    // TODO: await expect(page.getByText('VAT')).toBeVisible()
    ...
    // TODO: await expect(page).toHaveURL('/table-map')
    // TODO: await expect(page.getByTestId('table-tile-T1')).toHaveAttribute('data-status', 'Open')
  })
