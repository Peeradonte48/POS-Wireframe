...
test.describe('Takeaway Walk-in Flow', () => {
  ...
  test('create new takeaway order from queue tab', async ({ page }) => {
    ...
    await page.getByRole('tab', { name: 'Take-away' }).click()

    // Step 3: Assert TakeawayPanel is visible
    await expect(page.getByTestId('takeaway-panel')).toBeVisible()

    // Step 4: Click "รับออเดอร์ใหม่" (New Takeaway) button
    await page.getByRole('button', { name: 'รับออเดอร์ใหม่' }).click()

    // Step 5: Assert create takeaway dialog/modal appears
    await expect(page.getByRole('dialog')).toBeVisible()

    // Step 6: Enter customer name
    await page.getByLabel('ชื่อลูกค้า').fill('Somchai Test')
    ...
    await expect(page.getByTestId('takeaway-card-TK-001')).toBeVisible()
    await expect(page.getByTestId('takeaway-card-TK-001')).toContainText('Somchai Test')

    // Step 10: Assert order status starts as Taking
    await expect(page.getByTestId('takeaway-card-TK-001')).toContainText('Taking')
  })
  ...
  test('enter order items for takeaway', async ({ page }) => {
    ...
    await page.goto('/order/TK-001')

    // Step 2: Assert header shows TK-001 and customer name
    await expect(page.getByText('TK-001')).toBeVisible()
    await expect(page.getByText('Somchai Test')).toBeVisible()
    ...
    await page.getByTestId('spice-level-1').click()
    ...
    await expect(page.getByRole('button', { name: 'เช็คบิล' })).not.toBeVisible()

    // Step 8: Assert edit customer and cancel buttons are visible (Taking status)
    await expect(page.getByLabel('Edit customer')).toBeVisible()
    await expect(page.getByLabel('Cancel order')).toBeVisible()
  })
  ...
  test('complete takeaway payment', async ({ page }) => {
    ...
    await page.getByRole('tab', { name: 'Take-away' }).click()
    await expect(page.getByTestId('takeaway-card-TK-001')).toContainText('Sent')

    // Step 6: Navigate to payment for TK-001
    await page.goto('/payment/TK-001')
    ...
    await expect(page.getByRole('button', { name: 'Merge Bill' })).not.toBeVisible()
  ...
  test('order status advances to collected', async ({ page }) => {
    ...
    await expect(page.getByTestId('takeaway-card-TK-001')).toContainText('Collected')
    // Alternative: order may be hidden from active list after collection

    // Step 3: Assert active takeaway count badge updates (decreases or disappears)
    await expect(page.getByRole('tab', { name: 'Take-away' }).getByText('1')).not.toBeVisible()

    // Step 4: Verify queue.store reflects final state (Collected)
    // Assertion via localStorage or page evaluation:
    // const store = await page.evaluate(() => JSON.parse(localStorage.getItem('queue-store') ?? '{}'))
    // expect(store.state.orders['TK-001']?.status).toBe('Collected')
  })
