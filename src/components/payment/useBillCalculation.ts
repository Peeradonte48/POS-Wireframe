...
export function useBillCalculation(tableId: string): BillCalculationResult {
  ...
  const lineItems = useMemo(() => {
    const primaryItems = order
      ? order.rounds.flatMap((r) => r.items).filter((item) => item.status === 'sent' || item.status === 'ready' || item.status === 'served')
      : []
    ...
    const secondaryItems = mergedTableIds.flatMap((tid) => {
      ...
      if (!secOrder) return []
      return secOrder.rounds.flatMap((r) => r.items).filter((item) => item.status === 'sent' || item.status === 'ready' || item.status === 'served')
    })
  ...
  const tableOrders = useMemo(() => {
    if (isMerged) {
      return [tableId, ...mergedTableIds].map((tid) => ({
        ...
        ).filter((item) => item.status === 'sent' || item.status === 'ready' || item.status === 'served'),
      }))
    }
    return [
      {
