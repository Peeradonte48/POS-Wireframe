...
export function KdsTicketCard({ ticket, item, totalNonVoidedCount, hideHeader }: KdsTicketCardProps) {
  const { sendLineItem, acknowledgeCancelledItem } = useKdsStore()
  ...
  const isAcknowledged = (ticket.acknowledgedCancelLineIds ?? new Set<string>()).has(item.lineId)

  // Auto-remove acknowledged cancelled items after 15 seconds
  useEffect(() => {
    if (!isCancelled || !isAcknowledged) return
    const timeoutId = setTimeout(() => {
      sendLineItem(ticket.ticketId, item.lineId, totalNonVoidedCount ?? 1)
    }, 15000)
    return () => clearTimeout(timeoutId)
  }, [isCancelled, isAcknowledged, ticket.ticketId, item.lineId, totalNonVoidedCount, sendLineItem])
  ...
  function handleAcknowledgeCancel() {
    acknowledgeCancelledItem(ticket.ticketId, item.lineId)
    setShowCancelDialog(false)
  }

  // ── Acknowledged cancelled card (disabled, fading out) ──
  if (isCancelled && isAcknowledged) {
    return (
      <div
        className="bg-card border border-border rounded-xl overflow-hidden flex flex-col shrink-0 w-[240px] min-w-[240px] self-stretch opacity-30 pointer-events-none transition-opacity duration-500"
  ...
  // ── Cancelled card (clickable to acknowledge) ──
  if (isCancelled) {
    return (
      <>
        <div
        ...
        {showCancelDialog && (
          <KdsCancelInfoDialog
            onSave={handleAcknowledgeCancel}
