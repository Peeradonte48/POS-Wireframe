...
- [ ] **FLOOR-01**: Staff can view a floor plan with color + icon status per table (Open, Occupied, Reserved, Check Requested, Cleaning)
...
- [ ] **FLOOR-04**: Time-on-table dwell timer badge is visible on each occupied table tile
- [ ] **FLOOR-05**: Staff can assign a waiter to a table and add a table-level notes field (persists for the full visit)
...
- [ ] **KDS-01**: Kitchen staff can view a full-screen KDS (no sidebar, high-contrast) with ticket columns (New / In Progress / Ready)
- [ ] **KDS-02**: Kitchen staff can bump items and tickets, recall tickets, with elapsed timer per ticket
- [ ] **KDS-03**: Allergy/special request flags and post-send voided items (struck-through) are visible on KDS tickets
